...
<?php if ($nav) : ?>
<section
    class="introduce-nav spy-nav-menu border-b border-neutral-100 bg-white sticky z-10 -top--header-height overflow-hidden">
    <div data-aos="fade-up">
        <div class="container">
            <nav class="flex justify-center items-center relative">
                <ul class="flex items-center xl:gap-10 gap-6">
                    <?php foreach ($nav as $key => $value) : ?>
                    <li><a class="text-18px font-normal text-[#545454] transition-all relative py-3 block"
                            href="#section-<?= $key + 1 ?>">
                            <?= $value['title'] ?>
                        </a></li>
                    <?php endforeach; ?>
                ...
                <div
                    class="lavalamp absolute bottom-0 bg-primary-800 h-1px transition-all duration-700 xl:block hidden">
                </div>
            </nav>
        </div>
    </div>
